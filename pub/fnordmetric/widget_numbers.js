...
FnordMetric.js('jquery-1.6.1.min.js', function(){  
  ...
  function drawLayout(){
    for(n in widget_config.metrics){
      $('body').append(container=$('<div></div>').attr('class', 'numbers_container'));
      ...
      for(var k in widget_config.intervals){
        ...
        container.append($('<div></div>').addClass('number').attr('rel', u).attr('data',123).append(
  ...
  function updateValues(diff_factor){
    ...
    $('.number').each(function(){
      ...
      if(diff < 1){ diff=1; }
      console.log(diff);
      if(target_val > current_val-diff){ 
        still_running = true;
